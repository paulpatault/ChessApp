...
class ChessboardPredictor(object):
    ...
    def __init__(
        self, verbose=True, dir_=None, frozen_graph_path="saved_models/frozen_graph.pb"
    ):
        if dir_:
            frozen_graph_path = dir_
    ...
    def getPrediction(self, tiles):
        ...
        pieceNames = list(
            map(lambda k: "1" if k == 0 else labelIndex2Name(k), guessed)
        )  # exchange ' ' for '1' for FEN
        fen = "/".join(
            ["".join(pieceNames[i * 8 : (i + 1) * 8]) for i in reversed(range(8))]
        )
...
def getFEN(args):
    ...
    predictor = ChessboardPredictor(verbose=verbose, dir_=args.dir)
...
if __name__ == "__main__":
    ...
    web.open("http://www.fen-to-image.com/image/" + FEN)
